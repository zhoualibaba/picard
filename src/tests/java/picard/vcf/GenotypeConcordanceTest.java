...
public class GenotypeConcordanceTest {
    ...
    private static final List<Allele> alleles = Arrays.asList(Aref, C, G, T, AA, AAA, AAAA, AAAAA);
    ...
    public void testGenotypeConcordance(final File vcf1, final String sample1, final File vcf2, final String sample2,
        ...
        assertTextFilesEqualIgnoreCommentLines(outputSummaryFile, new File(TEST_DATA_PATH, expectedOutputFileBaseName + GenotypeConcordance.SUMMARY_METRICS_FILE_EXTENSION), "#");
        assertTextFilesEqualIgnoreCommentLines(outputDetailsFile, new File(TEST_DATA_PATH, expectedOutputFileBaseName + GenotypeConcordance.DETAILED_METRICS_FILE_EXTENSION), "#");
    }

    /**
     * Checks that the two files are the same length, and have the same content, otherwise throws a runtime exception.
     */
    private static void assertTextFilesEqualIgnoreCommentLines(final File f1, final File f2, final String commentLineStartString) {
        try {
            ...
            while (true) { // Continue while there are equal lines
                ...
                if (line1 == null) {     // End of file 1
                    ...
                    if (!line1.startsWith(commentLineStartString) || (!line2.startsWith(commentLineStartString))) {
    ...
    public void testGenotypeConcordanceDetails() throws Exception {
        ...
        nonZeroCounts.put(new TruthAndCallStates(TruthState.FILTERED, CallState.FILTERED), 40);
        ...
        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allHetTruthStates)), "1");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allHetCallStates)), "1");
        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allHomVarTruthStates)), "1");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allHomVarCallStates)), "1");
        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allVarTruthStates)), "1");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allVarCallStates)), "1");
        ...
        nonZeroCounts.put(new TruthAndCallStates(TruthState.FILTERED, CallState.FILTERED), 49);

        concordanceCounts = genotypeConcordance.getSnpCounter();
        assertNonZeroCountsAgree(concordanceCounts, nonZeroCounts);

        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allHetTruthStates)), "0.711538");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allHetCallStates)), "0.686869");
        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allHomVarTruthStates)), "0.766234");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allHomVarCallStates)), "0.730337");
        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allVarTruthStates)), "0.734807");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allVarCallStates)), "0.707447");
    }
    ...
    public void testGenotypeConcordanceDetailsWithIntervals() throws Exception {
        ...
        nonZeroCounts.put(new TruthAndCallStates(TruthState.FILTERED, CallState.FILTERED), 2);
        ...
        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allHetTruthStates)), "1");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allHetCallStates)), "1");
        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allHomVarTruthStates)), "?");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allHomVarCallStates)), "?");
        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allVarTruthStates)), "1");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allVarCallStates)), "1");
        ...
        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allHetTruthStates)), "1");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allHetCallStates)), "0.5");
        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allHomVarTruthStates)), "?");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allHomVarCallStates)), "?");
        Assert.assertEquals(fmt.format(concordanceCounts.getSensitivity(scheme, GenotypeConcordanceStates.allVarTruthStates)), "1");
        Assert.assertEquals(fmt.format(concordanceCounts.getPPV(scheme, GenotypeConcordanceStates.allVarCallStates)), "0.5");
    }
    ...
    public Object[][] genotypeConcordanceDetermineStateDataProvider() {
        return new Object[][]{
                ...
                {Allele.NO_CALL, Aref, TruthState.IS_MIXED, Aref, Aref, CallState.HOM_REF},
                {Aref, Allele.NO_CALL, TruthState.IS_MIXED, Aref, Aref, CallState.HOM_REF},
                {Allele.NO_CALL, Allele.NO_CALL, TruthState.NO_CALL, Aref, Aref, CallState.HOM_REF},

                {Aref, Aref, TruthState.HOM_REF, Allele.NO_CALL, Aref, CallState.IS_MIXED},
                {Aref, Aref, TruthState.HOM_REF, Aref, Allele.NO_CALL, CallState.IS_MIXED},
    ...
    public void testGenotypeConcordanceDetermineState(final Allele truthAllele1, final Allele truthAllele2, final TruthState expectedTruthState,
        ...
        // Test all orderings of genotypes
        _testGenotypeConcordanceDetermineState(truthAllele1, truthAllele2, expectedTruthState, callAllele1, callAllele2, expectedCallState);
        if (!callAllele1.equals(callAllele2)) {
            _testGenotypeConcordanceDetermineState(truthAllele1, truthAllele2, expectedTruthState, callAllele2, callAllele1, expectedCallState);
        }

        if (!truthAllele1.equals(truthAllele2)) {
            _testGenotypeConcordanceDetermineState(truthAllele2, truthAllele1, expectedTruthState, callAllele1, callAllele2, expectedCallState);
            if (!callAllele1.equals(callAllele2)) {
                _testGenotypeConcordanceDetermineState(truthAllele2, truthAllele1, expectedTruthState, callAllele2, callAllele1, expectedCallState);
            }
        }
    }

    private void _testGenotypeConcordanceDetermineState(final Allele truthAllele1, final Allele truthAllele2, final TruthState expectedTruthState,
                                                      final Allele callAllele1, final Allele callAllele2, final CallState expectedCallState) throws Exception {
        final Genotype truthGt = GenotypeBuilder.create(TRUTH_SAMPLE_NAME, Arrays.asList(truthAllele1, truthAllele2));
        final VariantContext truthVariantContext = new VariantContextBuilder("test", snpLoc, snpLocStart, snpLocStop, alleles).genotypes(truthGt).make();

        final Genotype callGt = GenotypeBuilder.create(CALL_SAMPLE_NAME, Arrays.asList(callAllele1, callAllele2));
        final VariantContext callVariantContext = new VariantContextBuilder("test", snpLoc, snpLocStart, snpLocStop, alleles).genotypes(callGt).make();
    ...
    public void testGenotypeConcordanceDetermineStateNull() throws Exception {
        ...
        testGenotypeConcordanceDetermineState(null, TruthState.HOM_REF, null, CallState.HOM_REF, 0, 0);
        testGenotypeConcordanceDetermineState(vc1, TruthState.HET_REF_VAR1, null, CallState.HOM_REF, 0, 0);
        testGenotypeConcordanceDetermineState(null, TruthState.HOM_REF, vc1, CallState.HET_REF_VAR1, 0, 0);
    }
    ...
    public void testGenotypeConcordanceDetermineStateFilter() throws Exception {
        ...
        final Genotype gt1 = GenotypeBuilder.create(TRUTH_SAMPLE_NAME, Arrays.asList(Aref, C));
        final VariantContext vcFiltered = new VariantContextBuilder("test", snpLoc, snpLocStart, snpLocStop, alleles).genotypes(gt1).filters(filters).make();

        final Genotype gt2 = GenotypeBuilder.create(TRUTH_SAMPLE_NAME, Arrays.asList(Aref, T));
        final VariantContext vcNotFiltered = new VariantContextBuilder("test", snpLoc, snpLocStart, snpLocStop, alleles).genotypes(gt2).make();

        testGenotypeConcordanceDetermineState(vcFiltered, TruthState.FILTERED, vcNotFiltered, CallState.HET_REF_VAR1, 0, 0);
        testGenotypeConcordanceDetermineState(vcNotFiltered, TruthState.HET_REF_VAR1, vcFiltered, CallState.FILTERED, 0, 0);
        testGenotypeConcordanceDetermineState(vcFiltered, TruthState.FILTERED, vcFiltered, CallState.FILTERED, 0, 0);
        ...
        final VariantContext vcGtFiltered = new VariantContextBuilder("test", snpLoc, snpLocStart, snpLocStop, alleles).genotypes(gt3).make();

        testGenotypeConcordanceDetermineState(vcGtFiltered, TruthState.FILTERED, vcNotFiltered, CallState.HET_REF_VAR1, 0, 0);
        testGenotypeConcordanceDetermineState(vcNotFiltered, TruthState.HET_REF_VAR1, vcGtFiltered, CallState.FILTERED, 0, 0);
        testGenotypeConcordanceDetermineState(vcGtFiltered, TruthState.FILTERED, vcGtFiltered, CallState.FILTERED, 0, 0);
    }
    ...
    public void testGenotypeConcordanceDetermineStateDp() throws Exception {
        ...
        final VariantContext vcNormal = new VariantContextBuilder("test", snpLoc, snpLocStart, snpLocStop, alleles).genotypes(gtNormal).make();

        final Genotype gtLowDp = new GenotypeBuilder(TRUTH_SAMPLE_NAME, Arrays.asList(Aref, C)).DP(4).make();
        final VariantContext vcLowDp = new VariantContextBuilder("test", snpLoc, snpLocStart, snpLocStop, alleles).genotypes(gtLowDp).make();
    ...
    public void testGenotypeConcordanceDetermineStateGq() throws Exception {
        ...
        final VariantContext vcLowGq = new VariantContextBuilder("test", snpLoc, snpLocStart, snpLocStop, alleles).genotypes(gtLowGq).make();
